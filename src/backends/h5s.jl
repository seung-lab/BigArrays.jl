...
function H5sContext(prefix::AbstractString, blksz::Vector=[])

# initialize the context
function init(;kw_args...)
    global compute_context = H5sContext(;kw_args)
end
...
function H5sBigArray()
    ...
    blksz::Vector = [4096, 4096, 512]
    chksz::Vector = [256, 256, 32]
    ba = H5sBigArray(prefix, blksz, chksz)
    updateconfig(ba)
    return ba
end
...
function H5sBigArray(fconfig::AbstractString)
    ...
    H5sBigArray(config[:prefix], config[:blksz], config[:chksz])
end
